...
const INSIGHTS = [
  { slug: 'insight-legaldst-rag', caseId: 'legaldst', title: 'Building production RAG with verified legal citations' },
  { slug: 'insight-cars24-ml', caseId: 'cars24-credit', title: 'In-house ML credit scoring at production volume' },
  { slug: 'insight-kotak-automation', caseId: 'kotak-sra', title: 'Automating 80–90% of Branch Service Requests at Kotak' },
  { slug: 'insight-tekion-dms', caseId: 'tekion-dms', title: 'Cloud-Native DMS: Replacing Legacy Dealership Systems' },
  { slug: 'insight-sdms-kafka', caseId: 'sdms', title: 'Monolith to Kafka: SDMS discount engine migration' },
  { slug: 'insight-iiot-ml', caseId: 'iiot', title: 'IIoT analytics and ML anomaly detection' },
  { slug: 'insight-devops-sre', caseId: 'verizon-mybiz', title: 'DevOps & SRE for regulated enterprise workloads' },
];

const PAGES = [
  { route: '/', source: 'src/pages/HomePage.jsx', file: 'home' },
  { route: '/about', source: 'src/content/pages/about.js', file: 'about' },
  { route: '/services', source: 'src/content/pages/services.js', file: 'services' },
  { route: '/casestudies', source: 'src/pages/CaseStudiesPage.jsx', file: 'casestudies' },
  { route: '/case-studies/:id', source: 'src/pages/CaseStudyPage.jsx + shared/cases.js', file: 'case-study-detail' },
  { route: '/industries', source: 'src/content/pages/industries.js', file: 'industries' },
  { route: '/industry-bfsi', source: 'src/content/pages/industry-bfsi.js', file: 'industry-bfsi' },
  { route: '/industry-telecom', source: 'src/content/pages/industry-telecom.js', file: 'industry-telecom' },
  { route: '/industry-automotive', source: 'src/content/pages/industry-automotive.js', file: 'industry-automotive' },
  { route: '/ai-hub', source: 'src/content/pages/ai-hub.js', file: 'ai-hub' },
  { route: '/ai-llm-rag', source: 'src/content/pages/ai-llm-rag.js', file: 'ai-llm-rag' },
  { route: '/ai-agentic', source: 'src/content/pages/ai-agentic.js', file: 'ai-agentic' },
  { route: '/ai-ml-platform', source: 'src/content/pages/ai-ml-platform.js', file: 'ai-ml-platform' },
  { route: '/ai-computer-vision', source: 'src/content/pages/ai-computer-vision.js', file: 'ai-computer-vision' },
  { route: '/devops-sre', source: 'src/content/pages/devops-sre.js', file: 'devops-sre' },
  { route: '/engagement', source: 'src/content/pages/engagement.js', file: 'engagement' },
  { route: '/methodology', source: 'src/content/pages/methodology.js', file: 'methodology' },
  { route: '/technology', source: 'src/content/pages/technology.js', file: 'technology' },
  { route: '/security', source: 'src/content/pages/security.js', file: 'security' },
  { route: '/locations', source: 'src/content/pages/locations.js', file: 'locations' },
  { route: '/leadership', source: 'src/content/pages/leadership.js', file: 'leadership' },
  { route: '/careers', source: 'src/content/pages/careers.js', file: 'careers' },
  { route: '/contact', source: 'src/content/pages/contact.js', file: 'contact' },
  { route: '/insights', source: 'src/content/pages/insights.js', file: 'insights' },
  { route: '/whitepapers', source: 'src/content/pages/whitepapers.js', file: 'whitepapers' },
  { route: '/newsroom', source: 'src/content/pages/newsroom.js', file: 'newsroom' },
];

const TESTIMONIALS = [
  { id: 'kotak', client: 'Kotak Mahindra Bank', role: 'Engineering Lead', caseId: 'kotak-lcrms' },
  { id: 'cars24', client: 'CARS24', role: 'Product Director', caseId: 'cars24-credit' },
  { id: 'legaldst', client: 'LegalDST', role: 'Founder & CEO', caseId: 'legaldst' },
];

const LEADERS = [
  'Rajesh Srinivasan', 'Anita Patel', 'Vikram Krishnan', 'Sarah Mitchell',
  'Deepak Prasad', 'Nisha Mehta', 'Arun Kumar', 'Lisa Chen',
];
...
function caseManifest(c) {
  ...
  const outcomes = c.outcomes.map((o, i) => `| ${i + 1} | ${o.replace(/\|/g, '\\|')} | Internal KPI / client email / load test | ☐ |`).join('\n');
...
for (const c of CASES) {
  ...
  write(path.join(dir, 'DROP-FILES-HERE.txt'), EVIDENCE_FILES.map((f) => f).join('\n') + '\n');
}
...
for (const t of TESTIMONIALS) {
  write(path.join(EVIDENCE, '04-testimonials', `${t.id}-PROOF-MANIFEST.md`), testimonialManifest(t));
}
...
const metricRows = [['claim_id', 'case_id', 'page_route', 'claim_text', 'claim_type', 'evidence_file', 'verified', 'owner', 'notes']];
let mid = 1;
for (const c of CASES) {
...
const homepageMetrics = [
  ['M-home-01', '', '/', '20+ Documented Engagements', 'stat', 'evidence/01-company/project-count-register.pdf', 'N', '', 'Must match PROJECT-REGISTER.csv count'],
  ['M-home-02', '', '/', '12+ Industries Served', 'stat', 'evidence/01-company/industry-count.pdf', 'N', '', ''],
  ['M-home-03', '', '/', '6 Global Regions', 'stat', 'evidence/01-company/office-addresses.pdf', 'N', '', ''],
  ['M-home-04', 'kotak-sra', '/', '80–90% Branch SR automated · Kotak', 'hero-metric', 'evidence/03-case-studies/kotak-sra/04-metrics-evidence.pdf', 'N', '', ''],
  ['M-home-05', '', '/', 'CMMI Level 3 processes', 'certification', 'evidence/02-certifications/cmmi-appraisal-certificate.pdf', 'N', '', ''],
];
metricRows.push(...homepageMetrics);

const csv = metricRows.map((row) => row.map((c) => `"${c}"`).join(',')).join('\n');
write(path.join(ROOT, 'METRICS-REGISTER.csv'), csv);

// CONTENT-INDEX.md
const pageList = PAGES.map((p) => `| \`${p.route}\` | [pages/${p.file}.md](pages/${p.file}.md) | ☐ |`).join('\n');
const caseList = CASES.map((c) => `| \`${c.id}\` | [evidence/03-case-studies/${c.id}/PROOF-MANIFEST.md](evidence/03-case-studies/${c.id}/PROOF-MANIFEST.md) | ☐ |`).join('\n');
