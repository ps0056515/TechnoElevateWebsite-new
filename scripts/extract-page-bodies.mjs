...
const pages = [
  ...
  "insight-tekion-dms",
  "insight-sdms-kafka",
  "insight-legaldst-rag",
  "insight-kotak-automation",
  "insight-iiot-ml",
  "insight-devops-sre",
  "insight-cars24-ml",
  "whitepapers",
...
function extractBody(html) {
  ...
  let body = html
    .slice(navEnd + '<div id="site-nav"></div>'.length, footerStart)
    .trim();
  body = body
...
function extractMeta(html) {
  const title =
    html.match(/<title>([^<]+)<\/title>/)?.[1]?.replace(" - Innovexce", "") ||
    "";
  const annMatch = html.match(
    /<div class="announcement[^"]*">([\s\S]*?)<\/div>/,
  );
  let announcement = null;
  if (annMatch) {
    const pill = annMatch[1].match(/<span class="pill">([^<]+)<\/span>/)?.[1];
    const text = annMatch[1].match(
      /<span class="pill">[^<]+<\/span>\s*<span>([^<]+)<\/span>/,
    )?.[1];
...
fs.writeFileSync(
  ...
  index
    .map((n) => `export * as ${n.replace(/-/g, "_")} from './${n}.js';`)
    .join("\n") + "\n",
);
